...
function register(){
	...
	if (count($errors) == 0) {
		$password = md5($password_1);//encrypt the password before saving in the database
			...
			$query = "INSERT INTO member (enroll_no, mem_name, mem_contact, mem_address, mem_username, password, mem_email, mem_type, reg_date, active) 
					  VALUES('$enroll_no', '$mem_name', '$mem_contact', '$mem_address', '$mem_username', '$password', '$mem_email', '$mem_type', now(),'No')";
...
function login2(){
	...
	$query1 = "SELECT * FROM member WHERE mem_username='$username' AND active='No'";
	$result1 = mysqli_query($db, $query1);
	 mysqli_num_rows($result1);
	if(mysqli_num_rows($result1) == 1){
		array_push($errors, "You haven't activated your account!");
	}
