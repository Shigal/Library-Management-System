    ...
    <section class="content">
        <div class="container-fluid">
			...
			<ol class="breadcrumb breadcrumb-bg-teal align-right">
				<div class="pull-left"><h2>SEARCH AND REQUEST BOOK</h2></div>
                ...
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <div class="card">
						...
						<form action="" method="post">
									<label>Title / Author / Category </label>
									<input type="textarea" required name="name"></input>
									<input type="submit" name="submit" value="Search Now!" class=" btn btn-primary "></input>
								</form>
						</div>
...
<?php
				...
				if(isset($_POST['submit'])){
					include('../db_conn.php');	
				$query4 = "SELECT * FROM book WHERE title LIKE '%{$_POST['name']}%' OR author LIKE '%{$_POST['name']}%' OR category LIKE '%{$_POST['name']}%'";	
				echo"<br>";
               ...
               echo" <table class='table table-bordered table-striped table-hover js-basic-example dataTable'>
                  <thead>
					...
					while($query4_row=mysql_fetch_assoc($query4_run))
										{
											$isbn = $query4_row['isbn'];
											$title = $query4_row['title'];											
											$author = $query4_row['author'];
											$edition = $query4_row['edition'];
											$publisher = $query4_row['publisher'];
											$year = $query4_row['year'];
											$status = $query4_row['status'];
											
											$book_no = $query4_row['book_no'];
											$_SESSION['book_no'] = $book_no;
											
											echo"<tr><td>".$isbn."</td><td>".$title."</td><td>".$author."</td><td>".$edition."</td><td>".$publisher."</td><td>".$year."</td><td>".$status."</td>";
											echo "<td>"?> <a href='search_request.php?id=<?php echo  $userRow['mem_id'] ?>'><i class="material-icons col-green">check_circle</i></a> <?php "</td>";
											echo "</tr>";											
										}		
					echo"</table>";
					}
