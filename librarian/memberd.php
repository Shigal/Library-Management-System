...
<body class="theme-red">
	...
	$query2 = "SELECT mem_type,COUNT(mem_id) FROM member GROUP BY mem_type";
		...
		if($query2_run = mysql_query($query2))
			{
				$count = mysql_num_rows($query2_run);
					if($count == null)
						...
						{
							echo "<table id=\"sampleTable\" class=\"table table-hover table-bordered\" width=\"100%\">
							 <thead>
			 <tr>
			   <th><center>Member Type</center></th>
			   <th><center>Total</center></th>
					...
					while($query2_row=mysql_fetch_assoc($query2_run))
						{
							$mem_type = $query2_row['mem_type'];
							$total = $query2_row['COUNT(mem_id)'];
   
							echo "<tr>";
						
							echo "<td><form action = \"member.php\" method = \"post\"><input type = \"hidden\" name = \"mem_type\" value = \" ".$mem_type."\" style=\"text-decoration:none\"/><input type=\"submit\" value=\" ".$mem_type."\" style=\"width:100%;border:none\"/></form></td>"; 
